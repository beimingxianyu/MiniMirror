...
TEST(config_system, main) {
  MM::ConfigSystem::ConfigSystem* config_system(MM::ConfigSystem::ConfigSystem::GetInstance());
...
//            std::string(CONFIG_DIR) + + "/init_config.ini");
  ...
  EXPECT_EQ(config_system->GetAllConfig().size(), 18);
  ...
  EXPECT_EQ(
      config_system->LoadConfigFromIni(MM::FileSystem::Path(config_system->GetConfig("config_dir") + "/init_config.ini")), MM::ExecuteResult::SUCCESS);
