...
template <typename RelationshipTrait, typename MultiTrait, typename KeyType,
          typename ObjectType, typename Hash = std::hash<KeyType>,
...
class HashTable {
 ...
 public:
  ...
  HashTable(const HashTable& other)
    ...
    other.LockAll();
    std::shared_lock<std::mutex> guard0{other.data_mutex0_, std::adopt_lock},
  ...
  HashTable(HashTable&& other) noexcept
    ...
    other.data_ = new Node[128]{};
    other.load_factor_ = 0.75;
    other.size_ = 0;
    other.bucket_count_ = 128;
  }
 ...
 public:
  ...
  std::pair<ObjectType&, bool> Insert(const ObjectType& object) {
    return Insert(object, MultiTrait{});
  }

  std::pair<ObjectType&, bool> Insert(ObjectType&& other) {
    return Insert(std::move(other), MultiTrait{});
  }

  template <typename... Args>
  std::pair<ObjectType&, bool> Emplace(Args&&... args) {
  ...
  std::vector<ObjectType*> EqualRange(const KeyType& key) {
    std::uint64_t hash_code = Hash{}(key);
    LockGuard guard{*this, hash_code, SharedLockType{}};
    ...
    while (first_node) {
      if (KeyEqual2(*(first_node->object_), key)) {
        result.emplace(first_node->object_);
      }
  ...
  std::vector<const ObjectType*> EqualRange(const KeyType& key) const {
    std::uint64_t hash_code = Hash{}(key);
    LockGuard guard{*this, hash_code, SharedLockType{}};
    ...
    if (new_bucket_size > bucket_count_) {
      ...
      for (std::uint64_t i = 0; i != bucket_count_; ++i) {
        if (data_[i].object_) {
          ...
          if (new_data[insert_pos] == nullptr) {
            new_data[insert_pos] = std::move(data_[i]);
            continue;
          }
          ...
          new_data_first_node->next_node_ = new Node{std::move(data_[i])};
        }
      }
 ...
 private:
  std::pair<ObjectType&, bool> Insert(const ObjectType& other, NotMulti) {
    RehashWhenNeed();
    ...
    std::uint64_t data_offset = hash_code % bucket_count_;
    if (data_[data_offset].object_ == nullptr) {
      data_[data_offset].object = std::make_unique<ObjectType>(other);
      return std::make_pair(*(data_[data_offset].object_), true);
    }

    Node* first_node = &data_[data_offset];
    while (first_node) {
      if (KeyEqual(*(first_node->object_), other)) {
        return std::make_pair(*(first_node->object_), false);
      }
  ...
  std::pair<ObjectType&, bool> Insert(const ObjectType& other, IsMulti) {
    RehashWhenNeed();
  ...
  std::pair<ObjectType&, bool> Insert(ObjectType&& other, NotMulti) {
    RehashWhenNeed();
  ...
  std::pair<ObjectType&, bool> Insert(ObjectType&& other, IsMulti) {
    RehashWhenNeed();
  ...
  template <typename... Args>
  std::pair<ObjectType&, bool> Emplace(NotMulti, Args&&... args) {
    return Insert(ObjectType{std::forward<Args>(args)...}, NotMulti{});
  }

  template <typename... Args>
  std::pair<ObjectType&, bool> Emplace(IsMulti, Args&&... args) {
  ...
  std::uint64_t GeObjectHash(const ObjectType& object) {
  ...
  bool KeyEqual2(const ObjectType& lhs, const KeyType& rhs) const {
    return KeyEqual2(lhs, rhs);
  }

  void RehashWhenNeed() {
    if (size_ > bucket_count_ * load_factor_) {
 ...
 private:
  Node* data_{new Node[128]{}};

  double load_factor_{0.75f};
  std::uint64_t size_{0};
  std::uint64_t bucket_count_{128};
...
template <typename ObjectType, typename Hash = std::hash<ObjectType>,
          ...
          typename Allocator = std::allocator<ObjectType>>
using ConcurrentSet = HashTable<FalseType, FalseType, ObjectType,
                                const ObjectType, Hash, Equal, Allocator>;

template <typename ObjectType, typename Hash = std::hash<ObjectType>,
          typename Equal = std::equal_to<ObjectType>,
          typename Allocator = std::allocator<ObjectType>>
using ConcurrentMultiSet = HashTable<FalseType, TrueType, ObjectType,
                                     const ObjectType, Hash, Equal, Allocator>;
