...
bool MM::RenderSystem::AllocatedImage::CanMapped() const {
  return CanBeMapped(image_data_info_.allocation_create_info_.usage_,
                            image_data_info_.allocation_create_info_.flags_);
}

bool MM::RenderSystem::AllocatedImage::IsTransformSrc() const {
  return IsTransformSrcImage(image_data_info_.image_create_info_.usage_);
}

bool MM::RenderSystem::AllocatedImage::IsTransformDest() const {
  return IsTransformDestImage(
...
MM::RenderSystem::AllocatedImage::AllocatedImage(
  ...
  if (auto if_result = CheckInitParametersWhenInitFromAnAsset(
               ...
               if_result.Exception(MM_ERROR_DESCRIPTION2("The parameters of MM::RenderSystem::AllocatedImage constructor are error.")).IsError()) {
    render_engine_ = nullptr;
    return;
  }
#endif
  ...
  const std::uint32_t recommend_mipmap_level =
      static_cast<uint32_t>(std::floor(
          std::log2(Utils::Max(vk_image_create_info->extent.width,
  ...
  Result<AllocatedBuffer> stage_allocated_buffer_result = LoadImageDataToStageBuffer(image);
  if (
    stage_allocated_buffer_result.IgnoreException().IsError()) {
    RenderResourceDataBase::Release();
    render_engine_ = nullptr;
    image_data_info_.Reset();
    return;
  }
  AllocatedBuffer& stage_allocated_buffer = stage_allocated_buffer_result.GetResult();


  Result<RenderResourceDataAttributeID> render_resource_data_attribute_ID_result = image_data_info_.GetRenderResourceDataAttributeID();
  ...
  if (auto if_result = InitImageFromAsset(stage_allocated_buffer, vk_image_create_info,
                              vma_allocation_create_info);
...
MM::RenderSystem::AllocatedImage::AllocatedImage(
  ...
  std::uint64_t format_size =
      GetVkFormatSize(vk_image_create_info->format);
...
MM::Result<MM::Nil> MM::RenderSystem::AllocatedImage::CheckImageHandler(
...
MM::RenderSystem::AllocatedImage::CheckInitParametersWhenInitFromAnAsset(
  ...
  if (auto if_result = CheckImageHandler(image_handler);
    if_result.Exception(MM_ERROR_DESCRIPTION2("Image asset handler is invalid.")).IsError()) {
    return ResultE<>{if_result.GetError().GetErrorCode()};
  }
...
MM::Result<MM::RenderSystem::AllocatedBuffer> MM::RenderSystem::AllocatedImage::LoadImageDataToStageBuffer(
    const AssetSystem::AssetType::Image* image_data) {
  const std::uint32_t graph_queue_index = render_engine_->GetGraphQueueIndex();
  Result<AllocatedBuffer> stage_allocated_buffer_result =
      render_engine_->CreateBuffer(
          ...
          nullptr);
  if (stage_allocated_buffer_result
          .Exception(MM_ERROR_DESCRIPTION2("Failed to create stage VkBuffer."))
          .IsError()) {
    return ResultE<>{stage_allocated_buffer_result.GetError().GetErrorCode()};
  }
  AllocatedBuffer& stage_allocated_buffer = stage_allocated_buffer_result.GetResult();
  ...
  if (auto if_result = ConvertVkResultToMMResult(
      vmaMapMemory(render_engine_->GetAllocator(),
                   ...
                   if_result.Exception(MM_ERROR_DESCRIPTION2("The vmaMapMemory operation failed, unable to complete "
                   "the pointer mapping operation.")).IsError()) {
    return ResultE<>{if_result.GetError().GetErrorCode()};
  }
  ...
  return ResultS{std::move(stage_allocated_buffer)};
}

MM::Result<MM::Nil> MM::RenderSystem::AllocatedImage::InitImageFromAsset(
  ...
  if (vk_image_create_info->mipLevels ==
    ...
    if (auto if_result = ConvertVkResultToMMResult(vmaCreateImage(render_engine_->GetAllocator(),
                               vk_image_create_info, vma_allocation_create_info,
                               &created_image, &created_allocation, nullptr));
                               if_result.Exception(MM_ERROR_DESCRIPTION2("Failed to create VkImage.")).IsError()) {
      return ResultE<>{if_result.GetError().GetErrorCode()};
    }
  } else {
    const VkImageCreateInfo temp_image_create_info =
  ...
  if (auto if_result =
      render_engine_->RunSingleCommandAndWait(
          CommandBufferType::GRAPH, false,
          [this_image = this, &created_image,
           &stage_allocated_buffer](AllocatedCommandBuffer& cmd) mutable -> Result<Nil> {
            if (auto if_result2 = BeginCommandBuffer(cmd);
              if_result2.Exception(MM_FATAL_DESCRIPTION2("Failed to begin VkCommandBuffer.")).IsError()) {
            ...
            if (auto if_result2 = EndCommandBuffer(cmd);
              if_result2.Exception(MM_FATAL_DESCRIPTION2("Failed to end VkCommandBuffer.")).IsError()) {
              return ResultE<>{if_result2.GetError().GetErrorCode()};
            }

            return ResultS<Nil>{};
          });
          if_result.Exception(MM_ERROR_DESCRIPTION2("Failed to copy stage buffer data to image.")).IsError() ||
...
void MM::RenderSystem::AllocatedImage::
    AddInitLayoutAndQueueIndexTransformCommands(AllocatedCommandBuffer& cmd, VkImage created_image) {
  const VkImageAspectFlags aspect_flags =
      ChooseImageAspectFlags(image_data_info_.image_create_info_.image_layout_);
  const VkImageMemoryBarrier2 image_memory_barrier2 = GetVkImageMemoryBarrier2(
...
void MM::RenderSystem::AllocatedImage::AddCopyStageBufferDataToImageCommands(
  ...
  const VkImageAspectFlags aspect_flags = ChooseImageAspectFlags(
      image_data_info_.image_create_info_.image_layout_);

  VkBufferImageCopy2 buffer_image_copy2 = GetVkBufferImageCopy2(
      nullptr, 0, 0, 0, GetVkImageSubResourceLayers(aspect_flags, 0, 0, 1),
...
MM::Result<MM::Nil> MM::RenderSystem::AllocatedImage::CheckInitParameters(
  ...
  if (auto if_result = CheckVkImageCreateInfo(vk_image_create_info);
    if_result.IgnoreException().IsError()) {
    return ResultE<>{if_result.GetError().GetErrorCode()};
  }

  if (auto if_result =
      CheckVmaAllocationCreateInfo(vma_allocation_create_info);
      if_result.IgnoreException().IsError()) {
    return ResultE<>{ErrorCode::INITIALIZATION_FAILED};
  }
...
void MM::RenderSystem::AllocatedImage::
  ...
  VkImageMemoryBarrier2 barrier = GetVkImageMemoryBarrier2(
  ...
  const VkDependencyInfo dependency_info =
      GetVkDependencyInfo(0, nullptr, 0, nullptr, 1, &barrier);
...
MM::Result<VkImageLayout> MM::RenderSystem::AllocatedImage::GetImageLayout(
  ...
  for (const auto& sub_resource :
    ...
    if (mipmap_level >=
      ...
      return ResultS{sub_resource.GetImageLayout()};
    }
  }
...
MM::Result<MM::RenderSystem::AllocatedImage> MM::RenderSystem::AllocatedImage::GetCopy(
    const std::string& new_name) const {
  if (IsValid()) {
    return ResultE<>{ErrorCode::OBJECT_IS_INVALID};
  }
  const QueueIndex transform_queue_index = render_engine_->GetTransformQueueIndex();
  VkImage new_image{nullptr};
  VmaAllocation new_allocation{nullptr};

  const VkImageCreateInfo image_create_info{GetVkImageCreateInfo(
  ...
  if (auto if_result = render_engine_->RunSingleCommandAndWait(
               ...
               if_result.Exception(MM_ERROR_DESCRIPTION2("Failed to copy data to new image.")).IsError()) {
    return ResultE<>{if_result.GetError().GetErrorCode()};
  }

  return ResultS{AllocatedImage{
      new_name, GetRenderResourceDataID(), render_engine_, image_data_info_,
      wrapper_.GetAllocator(), new_image, new_allocation}};
}

MM::Result<std::vector<MM::RenderSystem::AllocatedImage>> MM::RenderSystem::AllocatedImage::GetCopy(
  ...
  std::vector<VkImage> new_images(new_names.size());
  ...
  if (auto if_result = render_engine_->RunSingleCommandAndWait(
               CommandBufferType::TRANSFORM, false,
               std::vector<RenderResourceDataID>{GetRenderResourceDataID()},
               [this_image = this, new_images](AllocatedCommandBuffer& cmd) -> Result<Nil> {
                 if (this_image->GetSubResourceAttributes().size() == 1) {
                   return this_image->AddCopyImageCommandsWhenOneSubResource(
                       cmd, new_images);
                 } else {
  ...
  std::vector<AllocatedImage> new_allocated_images{};
  new_allocated_images.reserve(new_names.size());
  for (std::uint64_t i = 0; i != new_allocated_images.size(); ++i) {
    new_allocated_images.emplace_back(
        new_names[i],      GetRenderResourceDataID(), render_engine_,
        image_data_info_,  wrapper_.GetAllocator(),   new_images[i],
        new_allocations[i]);
  }

  return ResultS<std::vector<AllocatedImage>>{std::move(new_allocated_images)};
}
...
MM::RenderSystem::AllocatedImage::AddCopyImageCommandsWhenOneSubResource(
  ...
  // old image barrier
  barriers[0] = GetVkImageMemoryBarrier2(
      VK_PIPELINE_STAGE_2_TRANSFER_BIT, 0, VK_PIPELINE_STAGE_2_TRANSFER_BIT,
  ...
  // new image barrier
  barriers[1] = GetVkImageMemoryBarrier2(
      VK_PIPELINE_STAGE_2_TRANSFER_BIT, 0, VK_PIPELINE_STAGE_2_TRANSFER_BIT,
      VK_ACCESS_2_TRANSFER_WRITE_BIT,
      image_data_info_.image_create_info_.initial_layout_,
      sub_resource.GetImageLayout(), sub_resource.GetQueueIndex(),
  ...
  for (std::uint32_t i = 0; i != sub_resource.GetMipmapCount(); ++i) {
    ...
    image_copy_infos.emplace_back(GetVkImageCopy2(
        image_subresource_layers, image_subresource_layers, VkOffset3D{0, 0, 0},
        VkOffset3D{0, 0, 0}, GetImageExtent()));
  }

  const VkCopyImageInfo2 copy_image_info{GetVkCopyImageInfo2(
...
MM::RenderSystem::AllocatedImage::AddCopyImagCommandseWhenMultSubResource(
  ...
  for (const auto& sub_resource : GetSubResourceAttributes()) {
    const VkImageAspectFlags aspect_flags{
        ChooseImageAspectFlags(sub_resource.GetImageLayout())};
    const VkImageSubresourceRange sub_resource_range{GetVkImageSubresourceRange(
        aspect_flags, sub_resource.GetBaseMipmapLevel(),
        sub_resource.GetMipmapCount(), sub_resource.GetBaseArrayLevel(),
        sub_resource.GetArrayCount())};
    barriers.emplace_back(GetVkImageMemoryBarrier2(
  ...
  const auto sub_resource_all = GetSubResourceAttributes();
  ...
  for (std::uint64_t i = 0; i != sub_resource_all.size(); ++i) {
    ...
    if (i == sub_resource_all.size() - 1 ||
      ...
      VkCopyImageInfo2 copy_image_info{GetVkCopyImageInfo2(
          const_cast<VkImage>(GetImage()), new_image, nullptr, current_layout,
          current_layout, same_layout_count,
  ...
  const std::uint64_t sub_resource_count = GetSubResourceAttributes().size();
...
MM::RenderSystem::AllocatedImage::AddCopyImagCommandseWhenMultSubResource(
  ...
  const std::uint64_t sub_resource_count = GetSubResourceAttributes().size();
  const std::uint64_t new_images_count = new_images.size();
  for (std::uint64_t i = 0; i != sub_resource_count; ++i) {
    std::swap(barriers[i].srcQueueFamilyIndex, barriers[i].dstQueueFamilyIndex);
    barriers[i].srcAccessMask = VK_ACCESS_2_TRANSFER_WRITE_BIT;
    barriers[i].dstAccessMask = VK_ACCESS_2_TRANSFER_WRITE_BIT;
...
MM::RenderSystem::AllocatedImage::TransformSubResourceAttribute(
  ...
  if (auto if_result =
      render_engine_->RunSingleCommandAndWait(
          ...
          [this_image = this,
           &new_sub_resource_attribute](AllocatedCommandBuffer& cmd) -> Result<Nil> {
            ...
            for (const auto& old_sub_resource :
              ...
              const std::uint32_t old_sub_resource_mipmap_level_end =
                  old_sub_resource.GetBaseMipmapLevel() +
                  old_sub_resource.GetMipmapCount();
              std::uint32_t new_sub_resource_mipmap_level_end =
              ...
              if (new_sub_resource_attribute[new_sub_resource_index]
                ...
                const std::uint32_t transform_mipmap_level_offset =
                    old_sub_resource.GetBaseMipmapLevel() >
                            new_sub_resource_attribute[new_sub_resource_index]
                                .GetBaseMipmapLevel()
                        ? old_sub_resource.GetBaseMipmapLevel()
                        : new_sub_resource_attribute[new_sub_resource_index]
                              .GetBaseMipmapLevel();
                const std::uint32_t transform_mipmap_level_count =
                        old_sub_resource_mipmap_level_end <
                            ...
                            : new_sub_resource_mipmap_level_end -
                                  transform_mipmap_level_offset;
                const VkImageAspectFlags aspect_flag = ChooseImageAspectFlags(
                    new_sub_resource_attribute[new_sub_resource_index]
                        .GetImageLayout());
                current_to_transform_barriers.emplace_back(
                    GetVkImageMemoryBarrier2(
                ...
                transform_to_new_barriers.emplace_back(GetVkImageMemoryBarrier2(
                    VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                    VK_ACCESS_2_TRANSFER_READ_BIT,
                    VK_PIPELINE_STAGE_2_TRANSFER_BIT,
                    VK_ACCESS_2_TRANSFER_WRITE_BIT,
              ...
              while (new_sub_resource_mipmap_level_end <
                ...
                if (new_sub_resource_attribute[new_sub_resource_index]
                    ...
                    new_sub_resource_attribute[new_sub_resource_index]
                            .GetImageLayout() !=
                        old_sub_resource.GetImageLayout()) {
                  const std::uint32_t transform_offset =
                      old_sub_resource.GetBaseMipmapLevel() >
                              new_sub_resource_attribute[new_sub_resource_index]
                                  .GetBaseMipmapLevel()
                          ? old_sub_resource.GetBaseMipmapLevel()
                  ...
                  const std::uint32_t transform_size =
                      old_sub_resource_mipmap_level_end <
                              new_sub_resource_mipmap_level_end
                          ? old_sub_resource_mipmap_level_end - transform_offset
                          : new_sub_resource_mipmap_level_end -
                                transform_offset;
                  const VkImageAspectFlags aspect_flag = ChooseImageAspectFlags(
                      new_sub_resource_attribute[new_sub_resource_index]
                  ...
                  current_to_transform_barriers.emplace_back(
                      GetVkImageMemoryBarrier2(
                          ...
                          this_image->render_engine_->GetTransformQueueIndex(),
                          this_image->GetImage(),
                          GetVkImageSubresourceRange(aspect_flag,
                                                     transform_offset,
                                                     transform_size, 0, 1)));
            ...
            const VkDependencyInfo dependency_info1{GetVkDependencyInfo(
                0, nullptr, 0, nullptr, current_to_transform_barriers.size(),
                current_to_transform_barriers.data(), 0)};
            const VkDependencyInfo dependency_info2{GetVkDependencyInfo(
          ...
          if_result.Exception(MM_ERROR_DESCRIPTION2("Failed to transform sub resource attribute.")).IsError()) {
    return ResultE<>{if_result.GetError()};
          }
...
MM::RenderSystem::AllocatedImage::CheckTransformInputParameter(
    const std::vector<ImageSubResourceAttribute>& new_sub_resource_attribute)
    const {
  const QueueIndex graph_queue_index = render_engine_->GetGraphQueueIndex();
  const QueueIndex compute_queue_index = render_engine_->GetComputeQueueIndex();
  const QueueIndex transform_queue_index =
      render_engine_->GetTransformQueueIndex();
  const QueueIndex present_queue_index = render_engine_->GetPresentQueueIndex();
...
MM::RenderSystem::AllocatedImage::AllocatedImageWrapper::AllocatedImageWrapper(
    AllocatedImageWrapper&& other) noexcept
