...
void MM::RenderSystem::RenderEngine::FindSupportStorageImageFormat() {
  std::array<VkFormat, 247> candidates{
      VK_FORMAT_UNDEFINED,
  ...
  for (VkFormat format : candidates) {
    ...
    if (format_properties.linearTilingFeatures &
      ...
      support_storage_format_linear_.insert(format);
    }

    if (format_properties.optimalTilingFeatures &
        VK_FORMAT_FEATURE_STORAGE_IMAGE_BIT) {
      support_storage_format_optimal_.insert(format);
    }
  }
}
...
bool MM::RenderSystem::RenderEngine::FormatSupportStore(VkFormat format,
  ...
  if (tiling == VK_IMAGE_TILING_OPTIMAL) {
    return support_storage_format_optimal_.count(format);
  }
